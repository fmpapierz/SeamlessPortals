...
public class IPMcHelper {
    ...
    public static List<Pair<Portal, Vec3>> rayTracePortals(
        Level world,
        Vec3 start,
        Vec3 end,
        ...
        List<Pair<Portal, Vec3>> hits = new ArrayList<>();
        ...
        nearby.forEach(portal -> {
            if (filter == null || filter.test(portal)) {
                ...
                if (intersection != null) {
                    hits.add(Pair.of(portal, intersection));
                }
            }
        });
        
        hits.sort((pair1, pair2) -> {
            Vec3 intersection1 = pair1.getSecond();
            Vec3 intersection2 = pair2.getSecond();
    ...
    private static Pair<BlockHitResult, List<Portal>> rayTrace(
        ...
        List<Pair<Portal, Vec3>> rayTracedPortals =
            rayTracePortals(world, start, end, includeGlobalPortals, Portal::isInteractable);
        
        if (rayTracedPortals.isEmpty()) {
            return Pair.of(hitResult, portals);
        }
        
        Pair<Portal, Vec3> portalHit = rayTracedPortals.get(0);
        Portal portal = portalHit.getFirst();
        Vec3 intersection = portalHit.getSecond();
