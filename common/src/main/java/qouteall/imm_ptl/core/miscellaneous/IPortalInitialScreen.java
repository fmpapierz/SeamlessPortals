...
public class IPortalInitialScreen extends Screen {
    ...
    public IPortalInitialScreen(Runnable onClose) {
        ...
        // 26.2: Screen.minecraft/Screen.font are now FINAL (net/minecraft/client/gui/screens/Screen.java:66,:71),
        // initialized by super(Component) → this(Minecraft.getInstance(), Minecraft.getInstance().font, title)
        // (Screen.java:84-86) — exactly the values IP assigned here, so the explicit writes are dropped
        // (ducks-api-misc.md S21 widget layer; Screen ctor arity fold).
        ...
        // 26.2: StringWidget.alignCenter()/alignLeft()/alignRight() were REMOVED — the widget now renders
        // its text left-aligned within its bounds (StringWidget.visitLines uses x = getX(),
        // net/minecraft/client/gui/components/StringWidget.java:66; no alignment field remains). The
        // header/footer LinearLayout centering below provides the cell-level centering; the per-widget
        // text-align call is dropped (GONE API, cosmetic-only on this peripheral info screen).
    ...
    public void init() {
        ...
        // 26.2: StringWidget.alignCenter() removed (see ctor note) — dropped, cosmetic-only.
