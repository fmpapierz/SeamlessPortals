...
//        return Math.sqrt(1 - (1 - progress) * (1 - progress));
    ...
    public static void managePlayerRotationAndChangeGravity(
        ...
        if (portal.getRotation() != null) {
            ...
            float finalYaw = (float) (double) (pitchYaw.getSecond());
            float finalPitch = (float) (double) (pitchYaw.getFirst());
    ...
    private static void updateCamera(Minecraft client) {
        ...
        // 26.2: Camera.setup(BlockGetter, Entity, detached, mirrored, partialTick) is GONE
        // (api-map render-core G21). Its role — recompute the camera pos/rotation from the
        // player's (just-rewritten) pitch/yaw — is now Camera.update(DeltaTracker): it reads
        // level/entity already set, and reads detached/mirrored from options internally in the
        // private alignWithEntity(partialTicks). partialTick is taken from the frame's
        // DeltaTracker (equivalent to IP's RenderStates.getPartialTick()).
