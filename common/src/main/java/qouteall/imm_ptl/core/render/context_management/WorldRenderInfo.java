...
// S11-A port disposition: NEW (IP-add, current-mod-render §5). VERBATIM 1:1 from IP.
// 26.2 verification: PoseStack.Pose.pose()->Matrix4f (PoseStack.java:105) and .normal()->Matrix3f
// (:109) survive unchanged; Options.getEffectiveRenderDistance() survives. No API divergence.
// SIGN NOTE (D4.4): applyAdditionalTransformations does a MODEL-VIEW transform, not a
// projection/depth op — R5 reversed-Z does NOT touch this slice. matrixStack.last().pose().mul(matrix)
// is JOML column-form post-multiply M·v (identical idiom to FrontClipping, current-mod-render §1.6) —
// do NOT transpose it. The normal-matrix rescale to determinant 1 (pow(1/|det|,1/3)) is a scale-
// stripping normalization, not a winding flip. Held/inert until S13.
/**
