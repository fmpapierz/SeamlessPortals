...
// S11-A port disposition: NEW (IP-add, current-mod-render §5). VERBATIM 1:1 from IP EXCEPT one
// 26.2 rename: Camera.getPosition() -> Camera.position() (Camera.java:359) at getRenderingCameraPos.
// The mod's old boolean `isRenderingPortal` flag is REPLACED by this class's push/pop layer stack
// (current-mod-render §1.2). SIGN NOTE (D4.4): getRenderingCameraPos / getActiveClippingPlane compose
// portal.transformPoint / transformLocalVecNonScale — verbatim point/vector transforms; no depth or
// winding constant is introduced here (R5 reversed-Z lives in RendererUsingStencil, S12).
// Forward-refs (documented debt, S11(b) ledger): VisibleSectionDiscovery (U9 same-stage),
// PortalRenderer (U10, S12). Held/inert until S13.
...
public class PortalRendering {
    ...
    public static Vec3 getRenderingCameraPos() {
        // 26.2: Camera.getPosition() renamed to Camera.position() (Camera.java:359).
