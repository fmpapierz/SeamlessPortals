...
public class PortalExtension {
    ...
    private void readFromNbt(CompoundTag compoundTag) {
        ...
        // G2: CompoundTag.hasUUID/getUUID GONE -> read(name, UUIDUtil.CODEC) (Optional<UUID>,
        // same int-array wire format). Presence check = isPresent().
        Optional<UUID> reversePortalIdOpt = compoundTag.read("reversePortalId", UUIDUtil.CODEC);
        if (reversePortalIdOpt.isPresent()) {
            reversePortalId = reversePortalIdOpt.get();
        }
        else {
            reversePortalId = null;
        }
        Optional<UUID> flippedPortalIdOpt = compoundTag.read("flippedPortalId", UUIDUtil.CODEC);
        if (flippedPortalIdOpt.isPresent()) {
            flippedPortalId = flippedPortalIdOpt.get();
        }
        else {
            flippedPortalId = null;
        }
        Optional<UUID> parallelPortalIdOpt = compoundTag.read("parallelPortalId", UUIDUtil.CODEC);
        if (parallelPortalIdOpt.isPresent()) {
            parallelPortalId = parallelPortalIdOpt.get();
        }
        else {
