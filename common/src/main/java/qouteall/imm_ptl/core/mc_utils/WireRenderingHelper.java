...
public class WireRenderingHelper {
    ...
    // 26.2 reconstruction of the removed vanilla LevelRenderer.renderLineBox
    // (GONE on 26.2 — the VertexConsumer line-box helpers were replaced by the
    // immediate-mode Gizmos debug API; api-map render slice, verified absent across
    // mc262-ref). Draws the 12 edges of the axis-aligned box into `consumer` via the
    // 26.2 VertexConsumer builder (addVertex(Pose)/setColor/setNormal — all verified
    // present), reproducing the original wireframe. Private + identical arg order so the
    // single IP call site above is a one-token change (LevelRenderer. dropped).
    private static void renderLineBox(
        PoseStack matrixStack, VertexConsumer consumer,
        ...
        // 4 edges along X
        boxEdge(consumer, pose, x0, y0, z0, x1, y0, z0, 1, 0, 0, red, green, blue, alpha);
        boxEdge(consumer, pose, x0, y1, z0, x1, y1, z0, 1, 0, 0, red, green, blue, alpha);
        boxEdge(consumer, pose, x0, y0, z1, x1, y0, z1, 1, 0, 0, red, green, blue, alpha);
        boxEdge(consumer, pose, x0, y1, z1, x1, y1, z1, 1, 0, 0, red, green, blue, alpha);
        // 4 edges along Y
        boxEdge(consumer, pose, x0, y0, z0, x0, y1, z0, 0, 1, 0, red, green, blue, alpha);
        boxEdge(consumer, pose, x1, y0, z0, x1, y1, z0, 0, 1, 0, red, green, blue, alpha);
        boxEdge(consumer, pose, x0, y0, z1, x0, y1, z1, 0, 1, 0, red, green, blue, alpha);
        boxEdge(consumer, pose, x1, y0, z1, x1, y1, z1, 0, 1, 0, red, green, blue, alpha);
        // 4 edges along Z
        boxEdge(consumer, pose, x0, y0, z0, x0, y0, z1, 0, 0, 1, red, green, blue, alpha);
        boxEdge(consumer, pose, x1, y0, z0, x1, y0, z1, 0, 0, 1, red, green, blue, alpha);
        boxEdge(consumer, pose, x0, y1, z0, x0, y1, z1, 0, 0, 1, red, green, blue, alpha);
        boxEdge(consumer, pose, x1, y1, z0, x1, y1, z1, 0, 0, 1, red, green, blue, alpha);
    }
