...
public final class SeamCrossingRule {
    ...
    public static InPassBodyVerdict inPassRealBodyVerdict(
        ...
        double sideDot = collidingPortal.getNormal().dot(passClip.normal());
        // ★ PAIR SCOPE (2026-08-24, the tint round's UNTINTED verdict: "looking from source b
        // into dest a … a cart suddenly appears, cut off at the seam"). All four doors of a
        // seam cell are co-planar, so no side-dot can distinguish the two continuation PAIRS —
        // and a straddling cart's model overhang was drawn by the OTHER pair's window (the
        // pass's own real-entity path, untinted, ambient-clipped at the plane: exactly the
        // report). A seam-crossing body belongs only to the windows of ITS crossing pair: the
        // straddled face itself and that face's arrival counterpart. Everything else CULLs.
        boolean passInPair = renderingPortal == collidingPortal
            || renderingPortal == qouteall.imm_ptl.core.portal.PortalManipulation
                .findArrivalFacingPortal(collidingPortal)
            || collidingPortal == qouteall.imm_ptl.core.portal.PortalManipulation
                .findArrivalFacingPortal(renderingPortal);
        if (!passInPair) {
            return InPassBodyVerdict.CULL;
        }
        // ★ ANCHORED PHASE: KEEP IN BOTH OF ITS PAIR'S PASSES (2026-08-24; the two
        // single-sided variants each blinded one camera side — the transitional cart's carried
        // visual lies on the seam's rail line, both pair windows display their own half, and
        // each pass's plane-exact AMBIENT clip is the containment). Now pair-scoped above.
        if (isAnchoredAt(entity, collidingPortal)) {
