...
| B9 | `PortalForcerMixin` | `mixin/PortalForcerMixin.java` | transitive — vanilla `PortalForcer` never runs while `handlePortal` is cancelled (§2); dies at S20 (IP has zero PortalForcer references, core §1/§8) | DELETE (core §8) | ✅ TRANSITIVE — vanilla `PortalForcer` never runs while the §2 `handlePortal` cancel is active (both states) | |
