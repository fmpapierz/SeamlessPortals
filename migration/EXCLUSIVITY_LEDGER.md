...
| B4 | `HandleRespawnMixin` | `mixin/client/HandleRespawnMixin.java` | crossing-keyed paths idle flag-ON (IP's crossing never triggers `handleRespawn`), **but its player-reuse also runs on death/vanilla respawns — S13 must decide the gate scope explicitly** (core §9: death respawns revert to vanilla) | DELETE (core §9) | ✅ DECISION: **NO gate** (S13) → **SUPERSEDED S17 (wf_5183007f-fee): GATED `!entityPortals` at beforeRespawn HEAD** — the S13 fear ("gating breaks flag-ON vanilla respawns") no longer holds: flag-ON vanilla respawns ride vanilla + IP's onSetWorld cleanup (S16.2-verified, crouch-hatch live-proven); the judge verified every flag-ON-live branch inert (gated writers; the RemoteChunkDataPayload branch has NO sender anywhere in the tree). Defensive gate per the 4x-failed inert-by-dormancy record; the sibling handlers are transition-state-keyed off the gated HEAD → transitively inert flag-ON | |
...
~~`ServerLevelFireSpreadMixin`~~ (**S17 sweep: GATED `!entityPortals`** — block-era-registry-keyed,
structurally inert flag-ON on all platforms), `FireworkRocketEntityAccessor` (F2),
`LivingEntityHurtAccessor` (F3) (core §8); `ChunkPacketGuardMixin`,
~~`ClientPacketListenerAddEntityAdoptMixin`~~ (**S17 sweep: DEFENSIVE-GATED `!entityPortals`** —
block-era adoption serves the block-era crossing only),
