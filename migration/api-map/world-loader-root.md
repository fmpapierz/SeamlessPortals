...
| `Level.getBiomeManager()` + `biomeZoomSeed` (:455) | `Level.java:992`; `private final long biomeZoomSeed` `BiomeManager.java:16` (private — widening needed, as on 1.21.3). **S10-B DONE:** IP widens this via `imm_ptl.accesswidener`, so this mod grew `seamlessportals.accesswidener` (`accessible field .../BiomeManager biomeZoomSeed J`) — but on this NeoForge ModDevGradle classpath the AT (`accesstransformer.cfg`), NOT the AW, is the compile-time widener, so a **paired AT entry is required** (`public net.minecraft.world.level.biome.BiomeManager biomeZoomSeed`; read-only, no `-f` de-final, mirroring the `server`/`storageSource`/`ChunkMap$TrackedEntity` precedent). **RECURRING:** every field/type widening in this port needs BOTH the AW entry (IP-faithful) AND the paired AT entry (compile-time), or the probe/S13-closure fails private-access while shipping stays green (held). |
