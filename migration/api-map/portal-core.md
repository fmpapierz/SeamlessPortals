...
**S6 amendment (2026-07-14, FixGap from the `-Pip_scc_closed=true` probe + Fable verify on the landed U4 slice):** +1 CHANGED — the server-damage entry point `Entity.hurtServer(ServerLevel, DamageSource, float)` is now `public abstract` (`Entity.java:1923`; the client half `hurtClient(DamageSource)` :1925 is concrete), so the two direct-`Entity` subclasses in this slice, `Portal` and `LoadingIndicatorEntity`, MUST implement it — see the `[S6]` row appended to §2.2. IP's Portal/LoadingIndicatorEntity carried **no** `hurt`/`hurtServer` override (they relied on the concrete 1.21.3 `Entity.hurt` no-op + validity-based removal), so this is a research-corpus gap, not an IP semantic. Post-amendment tally: **3 GONE, 25 CHANGED, 108 SAME, 5 FABRIC-API**. Rows below carry an "Applied S6" tag. NOTE — the disk-NBT bridge mechanism chosen at S6 (nest the verbatim IP `CompoundTag` under one key via `CompoundTag.CODEC`, rather than the C1-suggested top-level `TagValueOutput.buildResult()`) is a **governed port-note deviation, not an api-map change**: C1 stays exactly as written and the render-relevant sync wire is unchanged; the rationale lives in `migration/port-notes/S06-portal-core.md §5.2`.
...
| **[S6]** `Entity.hurt(DamageSource, float)` inherited no-op — IP `Portal`/`LoadingIndicatorEntity` add **no** damage override (they inherit `Entity`'s concrete 1.21.3 `hurt` and are removed via validity checks, never damaged) | **CHANGED (now abstract — FixGap).** The 1.21.3 concrete `Entity.hurt` server path split; `public abstract boolean hurtServer(ServerLevel level, DamageSource source, float damage)` (`Entity.java:1923`) is now unimplemented on the base class. `hurtClient(DamageSource)` (`:1925`) stays concrete. Every **direct** `Entity` subclass must implement `hurtServer` — `Portal` and `LoadingIndicatorEntity` are direct subclasses; `Mirror`/`BreakableMirror`/`EndPortalEntity` extend `Portal` and inherit its impl. | **Applied S6:** both add `@Override public boolean hurtServer(ServerLevel, DamageSource, float) { return false; }` — faithful to IP's inherited no-damage behavior and matching vanilla's own non-damageable marker entity `Display#hurtServer` (`Display.java:140`, also `return false`). Not a Fabric/loader seam; pure vanilla-abstract satisfaction. Recorded here per D4.2 ledger governance (was previously only an inline code comment). |
